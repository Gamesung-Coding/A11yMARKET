...
function CartPage() {
      ...
      prev
        .map((group) => ({
          ...
          items: group.items.filter(
            (item) => !(group.sellerId === sellerId && item.id === itemId)
          ),
        }))
        .filter((group) => group.items.length > 0)
    );
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-10">
        <p className="text-center text-xs text-gray-600 mb-6">
          <span className="font-semibold text-gray-900">01 장바구니</span> &gt; 02
          주문결제 &gt; 03 주문완료
        </p>
        <div className="h-64 border border-gray-200 flex items-center justify-center text-gray-500">
          장바구니에 담긴 상품이 없습니다.
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10 text-[#333333]">
      {/* 상단 단계 표시 */}
      <p className="text-center text-xs text-gray-600 mb-6">
        <span className="font-semibold text-gray-900">01 장바구니</span> &gt; 02
        주문결제 &gt; 03 주문완료
      ...
      {cartGroups.map((group) => {
        ...
        return (
          <section
            key={group.sellerId}
            className="mb-5 rounded-3xl border border-gray-200 overflow-hidden bg-[#f4f4f4]"
          >
            {/* 판매자명 헤더 영역 */}
            ...
            {group.items.map((item) => (
              <div
                key={item.id}
                className="flex items-center px-8 py-5 border-b border-gray-200 bg-white last:border-b-0"
              >
                {/* 체크박스 */}
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 border-gray-400 rounded-sm mr-4"
                />

                {/* 썸네일 */}
                <div className="w-16 h-16 bg-gray-200 mr-6 flex-shrink-0" />
                ...
                <div className="w-20 text-center text-xs text-gray-700">
                  ...
                  { <div className="inline-flex items-center border border-gray-300">
                    <button
                      type="button"
                      onClick={() =>
                        handleChangeQty(group.sellerId, item.id, -1)
                      }
                      className="px-2 text-xs border-r border-gray-300"
                    >
                      -
                    </button>
                    ...
                    <button
                      ...
                      className="px-2 text-xs border-l border-gray-300"
                    >
                      +
                    </button>
                  </div> }
                ...
                {/* 가격 */}
                <div className="w-32 text-right text-sm font-medium mr-6">
...
>
            ...
            {/* 하단 판매자별 합계 */}
            <div className="px-8 py-4 bg-[#f4f4f4] text-center text-xs text-gray-600">
              상품 {formatPrice(groupProductTotal)} + 배송비{" "}
      ...
      {/* 결제 정보 박스 */}
      <div className="border border-gray-300 bg-[#f9f9f9] px-10 py-10 mb-6">
        <div className="flex justify-end">
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between gap-8 pt-3 mt-2 border-t border-gray-300">
              <span className="font-semibold">총 결제 예상 금액</span>
              <span className="font-bold">
                {formatPrice(totals.expectedTotal)}
              </span>
            </div>
          </div>
        </div>
