...
const a11ySlice = createSlice({
    ...
    reducers: {
        ...
        toggleScreenReader(state) { // 스크린 리더 on/off
            state.screenReader = !state.screenReader
        },   
        toggleSmartContrast(state) { // 스마트 대비 on/off
            state.smartContrast = !state.smartContrast
        },  
        toggleHighlightLinks(state) { // 링크 강조 on/off
            state.highlightLinks = !state.highlightLinks
        }, 
        toggleCursorHighlight(state) { // 마우스 커서 강조 on/off
            state.cursorHighlight = !state.cursorHighlight
        },

        resetAll(state) {   //모든 옵션 초기화
